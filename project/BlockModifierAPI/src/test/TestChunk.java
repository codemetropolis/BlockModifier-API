...
//      TODO test other attributes of the object
    ...
    public void testSetSpawnerSubstanceSpawnerId() {
        ...
        assertEquals("A spawner id-jának értéke nem jól állítódott be!", "MobSpawner", getTileEntites(chunk).getSubtags()[0].getSubtagByName("id").getValue());
    }
    ...
    public void testSetSpawnerSubstanceSpawnerSpawnDataEntityId() {
        ...
        assertEquals("A spawner SpawnData-jának entitásában az id értéke nem megfelelő!", "Zombie", getTileEntites(chunk).getSubtags()[0].getSubtagByName("SpawnData").getSubtagByName("entity").getSubtagByName("id").getValue());
    }
    ...
    public void testSetSpawnerSubstanceSpawnerMultipleSpawnerDifferentPlace() {
        ...
        assertEquals("A spawner SpawnData-jának entitásában az id értéke megváltozott, pedig nem kellett volna!", "Zombie", getTileEntites(chunk).getSubtags()[0].getSubtagByName("SpawnData").getSubtagByName("entity").getSubtagByName("id").getValue());
        assertEquals("A spawner SpawnData-jának entitásában az id értéke nem megfelelően állítódott be!", "Spider", getTileEntites(chunk).getSubtags()[1].getSubtagByName("SpawnData").getSubtagByName("entity").getSubtagByName("id").getValue());
    ...
    public void testSetSpawnerSubstanceSpawnerMultipleSpawnerSamePlace() {
        ...
        assertEquals("A spawner SpawnData-jának entitásában az id értéke nem változott, pedig kellett volna!", "Spider", getTileEntites(chunk).getSubtags()[0].getSubtagByName("SpawnData").getSubtagByName("entity").getSubtagByName("id").getValue());
